...
Simulates a classic M/M/c queueing system. Essentially re-simulates
FIFO functionality - kept for reference purposes
...
from src import calculate_metrics, default_metric_output, build_processed_df, calculate_utilization

def mmc(num_servers: int, original_df: pd.DataFrame = None, capacity: Optional[int] = None)\
        -> Tuple[pd.DataFrame, pd.DataFrame, list[list[tuple[datetime, datetime]]], dict]:
    """
    Simulate a multi-server M/M/c queue without blocking
    ...
    # Sort arrival times
    df: pd.DataFrame = df.sort_values(by='ArrivalTime').copy()
    ...
    for idx, row in df.iterrows():
        ...
        # Actual start time (arrival + availability)
