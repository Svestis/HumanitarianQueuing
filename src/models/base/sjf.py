...
Shortest Job First scheduling where ndividuals are served based on the shortest transaction time first
...
def sjf(num_servers: int, original_df: pd.DataFrame = None, capacity: Optional[int] = None)\
        -> Tuple[pd.DataFrame, pd.DataFrame, list[list[tuple[datetime, datetime]]], dict]:
    """
    Shortest Job First scheduling (based on TransactionTime).
    ...
    # Sort arrival times
    df: pd.DataFrame = df.sort_values(by=['ArrivalTime', 'TransactionTime']).reset_index(drop=True)
    ...
    while current_idx < len(df) or queue:
        # Add arrivals to queue if they have arrived
        while current_idx < len(df) and df.loc[current_idx, 'ArrivalTime'] <= current_time:
            if capacity is not None and len(in_system) >= capacity:
                ...
                queue.append(df.loc[current_idx].copy())
            current_idx += 1

        for sid in range(num_servers):
            if queue and server_end_times[sid] <= current_time:
                # Choose the shortest transaction time
                queue.sort(key=lambda x: (x['TransactionTime'], x['ArrivalTime']))
                row = queue.pop(0)
