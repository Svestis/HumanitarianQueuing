...
Validating mmc.py functionality. Same logic as fifo for benchmarking
...
def test_structure(test_data: pd.DataFrame) -> None:
    ...
    :return: None
    """
    df_result, util_df, _, metrics = mmc(original_df=test_data, num_servers=2, capacity=5)
...
def test_metrics_keys(test_data: pd.DataFrame) -> None:
    ...
    :return: None
    """
    _, _, _, metrics = mmc(original_df=test_data, num_servers=2, capacity=5)
...
def test_utilization_structure(test_data: pd.DataFrame) -> None:
    ...
    :return: None
    """
    _, util_df, _, _ = mmc(original_df=test_data, num_servers=2, capacity=5)
...
def test_all_blocked_() -> None:
    ...
    processed, _, _, metrics = mmc(original_df=df, num_servers=1, capacity=0)
...
def test_no_waiting(test_data: pd.DataFrame):
    ...
    :return: None
    """
    processed, _, _, metrics = mmc(original_df=test_data, num_servers=5, capacity=10)
...
def test_metrics() -> None:
    ...
    _, util_df, _,metrics = mmc(original_df=df, num_servers=1, capacity=1)
...
def test_metric_ranges() -> None:
    ...
    _, _, _, metrics = mmc(original_df=df, num_servers=2, capacity=3)
...
def test_empty_input() -> None:
    ...
    processed, util_df, _, metrics = mmc(original_df=df, num_servers=1)
...
def test_partial_blocking() -> None:
    ...
    processed, _, _, metrics = mmc(original_df=df, num_servers=1, capacity=2)
...
def test_simultaneous_arrivals() -> None:
    ...
    df['ArrivalTime'] = pd.to_datetime(df['ArrivalTime'], format='%H:%M')
    processed, _, _, metrics = mmc(original_df=df, num_servers=2, capacity=3)
    assert not processed.empty
...
def test_output_types(test_data: pd.DataFrame) -> None:
    ...
    :return: None
    """
    processed, util_df, server_logs, metrics = mmc(original_df=test_data, num_servers=2)
