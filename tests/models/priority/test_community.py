...
Validating community.py functionality
...
import pytest
import pandas as pd
from src import community_aware
...
def test_structure(test_data: pd.DataFrame) -> None:
    ...
    :return: None
    """
    df_result, util_df, _, metrics = community_aware(original_df=test_data, num_servers=2, capacity=5,
                                                     community_priority={"CommunityA": 2, "CommunityB": 1})
...
def test_metrics_keys(test_data: pd.DataFrame) -> None:
    ...
    :return: None
    """
    _, _, _, metrics = community_aware(original_df=test_data, num_servers=2, capacity=5,
                                       community_priority={"CommunityA": 2, "CommunityB": 1})
...
def test_utilization_structure(test_data: pd.DataFrame) -> None:
    ...
    :return: None
    """
    _, util_df, _, _ = community_aware(original_df=test_data, num_servers=2, capacity=5,
                                       community_priority={"CommunityA": 2, "CommunityB": 1})
...
def test_all_blocked_() -> None:
    ...
    processed, _, _, metrics = community_aware(original_df=df, num_servers=1, capacity=0,
                                               community_priority={"CommunityA": 2, "CommunityB": 1})
...
def test_no_waiting(test_data: pd.DataFrame):
    ...
    :return: None
    """
    processed, _, _, metrics = community_aware(original_df=test_data, num_servers=5, capacity=10,
                                               community_priority={"CommunityA": 2, "CommunityB": 1})
...
def test_metrics() -> None:
    ...
    _, util_df, _,metrics = community_aware(original_df=df, num_servers=1, capacity=1,
                                            community_priority={"CommunityA": 2, "CommunityB": 1})
...
def test_metric_ranges() -> None:
    ...
    _, _, _, metrics = community_aware(original_df=df, num_servers=2, capacity=3,
                                       community_priority={"CommunityA": 2, "CommunityB": 1})
...
def test_empty_input() -> None:
    ...
    processed, util_df, _, metrics = community_aware(original_df=df, num_servers=1,
                                                     community_priority={"CommunityA": 2, "CommunityB": 1})
...
def test_partial_blocking() -> None:
    ...
    processed, _, _, metrics = community_aware(original_df=df, num_servers=1, capacity=2,
                                               community_priority={"CommunityA": 2, "CommunityB": 1})
...
def test_simultaneous_arrivals() -> None:
    ...
    df['ArrivalTime'] = pd.to_datetime(df['ArrivalTime'], format='%H:%M')
    processed, _, _, metrics = community_aware(original_df=df, num_servers=2, capacity=3,
                                               community_priority={"CommunityA": 2, "CommunityB": 1})
    assert not processed.empty
...
def test_output_types(test_data: pd.DataFrame) -> None:
    ...
    :return: None
    """
    processed, util_df, server_logs, metrics = community_aware(original_df=test_data, num_servers=2,
                                                               community_priority={"CommunityA": 2, "CommunityB": 1})
...
def test_community_priority_serving_order() -> None:
    """Ensure higher priority communities are served first at the same arrival time.
    :return: None"""
    df = pd.DataFrame({
        'ID': [1, 2],
        'Gender': ['M', 'F'],
        'Age': [30, 25],
        'TransactionTime': [10, 10],
        'VulnerabilityScore': [0.1, 0.1],
        'ArrivalTime': ['08:00', '08:00'],
        'Community': ['CommunityB', 'CommunityA']  # A has higher priority
    })
    df['ArrivalTime'] = pd.to_datetime(df['ArrivalTime'], format='%H:%M')

    result, *_ = community_aware(original_df=df, num_servers=1, capacity=2,
                                 community_priority={"CommunityA": 1, "CommunityB": 2})

    first_served_id = result.sort_values('StartTime').iloc[0]['ID']
    assert first_served_id == 2  # CommunityA gets priority

def test_random_community_assignment(monkeypatch) -> None:
    """Test fallback logic when Community is not present in the input data.
    :return: None"""
    df = pd.DataFrame({
        'ID': [1, 2, 3],
        'Gender': ['M', 'F', 'M'],
    ...
    monkeypatch.setattr(np.random, 'choice', lambda a, size: ['CommunityA'] * size)

    result, *_ = community_aware(original_df=df, num_servers=1, capacity=5,
                                 community_priority={"CommunityA": 1, "CommunityB": 2})

    assert 'Community' in result.columns
    assert all(result['Community'] == 'CommunityA')
