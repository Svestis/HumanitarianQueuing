...
Validating fair.py functionality
...
def test_structure(test_data: pd.DataFrame) -> None:
    ...
    :return: None
    """
    df_result, util_df, _, metrics = fair(original_df=test_data, num_servers=2, capacity=5)
...
def test_metrics_keys(test_data: pd.DataFrame) -> None:
    ...
    :return: None
    """
    _, _, _, metrics = fair(original_df=test_data, num_servers=2, capacity=5)
...
def test_utilization_structure(test_data: pd.DataFrame) -> None:
    ...
    :return: None
    """
    _, util_df, _, _ = fair(original_df=test_data, num_servers=2, capacity=5)
...
def test_all_blocked_() -> None:
    ...
    processed, _, _, metrics = fair(original_df=df, num_servers=1, capacity=0)
...
def test_no_waiting(test_data: pd.DataFrame):
    ...
    :return: None
    """
    processed, _, _, metrics = fair(original_df=test_data, num_servers=5, capacity=10)
...
def test_metrics() -> None:
    ...
    _, util_df, _,metrics = fair(original_df=df, num_servers=1, capacity=1)
...
def test_metric_ranges() -> None:
    ...
    _, _, _, metrics = fair(original_df=df, num_servers=2, capacity=3)
...
def test_empty_input() -> None:
    ...
    processed, util_df, _, metrics = fair(original_df=df, num_servers=1)
...
def test_partial_blocking() -> None:
    ...
    processed, _, _, metrics = fair(original_df=df, num_servers=1, capacity=2)
...
def test_simultaneous_arrivals() -> None:
    ...
    df['ArrivalTime'] = pd.to_datetime(df['ArrivalTime'], format='%H:%M')
    processed, _, _, metrics = fair(original_df=df, num_servers=2, capacity=3)
    assert not processed.empty
...
def test_output_types(test_data: pd.DataFrame) -> None:
    ...
    :return: None
    """
    processed, util_df, server_logs, metrics = fair(original_df=test_data, num_servers=2)
    assert isinstance(processed, pd.DataFrame)
    assert isinstance(util_df, pd.DataFrame)
    assert isinstance(server_logs, list)
    assert isinstance(metrics, dict)

def test_fair_distribution() -> None:
    """
    All simultaneous arrivals treated fairly
    :return: None
    """
    df = pd.DataFrame({
        'ID': range(1, 11),
        ...
        'Age': [30] * 10,
        'TransactionTime': [10] * 10,
        'VulnerabilityScore': [0.5] * 10,
        'ArrivalTime': ['08:00'] * 10
    })
    df['ArrivalTime'] = pd.to_datetime(df['ArrivalTime'], format='%H:%M')
    processed, *_ = fair(original_df=df, num_servers=2)
    served_ids = processed.sort_values('StartTime')['ID'].tolist()
    assert set(served_ids) == set(df['ID'])

def test_no_starvation() -> None:
    """
    Individuals should not wait indefinitely
    :return: None
    """
    df = pd.DataFrame({
        'ID': range(1, 6),
        'Gender': ['M'] * 5,
        'Age': [30] * 5,
        'TransactionTime': [10] * 5,
        'VulnerabilityScore': [0.5] * 5,
        'ArrivalTime': ['08:00'] * 5
    })
    df['ArrivalTime'] = pd.to_datetime(df['ArrivalTime'], format='%H:%M')
    processed, *_ = fair(original_df=df, num_servers=1)
    max_wait = (processed['StartTime'] - processed['ArrivalTime']).dt.total_seconds().max() / 60
    assert max_wait < 60

def test_fairness() -> None:
    """Test fairness under backlog conditions
    :return: None"""
    df = pd.DataFrame({
        'ID': range(1, 101),
        'Gender': ['F'] * 100,
        'Age': [30] * 100,
        'TransactionTime': [5] * 100,
        'VulnerabilityScore': [0.1] * 100,
        'ArrivalTime': ['08:00'] * 100
    })
    df['ArrivalTime'] = pd.to_datetime(df['ArrivalTime'], format='%H:%M')
    processed, *_ = fair(original_df=df, num_servers=2)
