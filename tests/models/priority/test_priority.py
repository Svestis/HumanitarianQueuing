...
Validating priority.py functionality
...
def test_structure(test_data: pd.DataFrame) -> None:
    ...
    :return: None
    """
    df_result, util_df, _, metrics = priority(original_df=test_data, num_servers=2, capacity=5)
...
def test_metrics_keys(test_data: pd.DataFrame) -> None:
    ...
    :return: None
    """
    _, _, _, metrics = priority(original_df=test_data, num_servers=2, capacity=5)
...
def test_utilization_structure(test_data: pd.DataFrame) -> None:
    ...
    :return: None
    """
    _, util_df, _, _ = priority(original_df=test_data, num_servers=2, capacity=5)
...
def test_all_blocked_() -> None:
    ...
    processed, _, _, metrics = priority(original_df=df, num_servers=1, capacity=0)
...
def test_no_waiting(test_data: pd.DataFrame):
    ...
    :return: None
    """
    processed, _, _, metrics = priority(original_df=test_data, num_servers=5, capacity=10)
...
def test_metrics() -> None:
    ...
    _, util_df, _,metrics = priority(original_df=df, num_servers=1, capacity=1)
...
def test_metric_ranges() -> None:
    ...
    _, _, _, metrics = priority(original_df=df, num_servers=2, capacity=3)
...
def test_empty_input() -> None:
    ...
    processed, util_df, _, metrics = priority(original_df=df, num_servers=1)
...
def test_partial_blocking() -> None:
    ...
    processed, _, _, metrics = priority(original_df=df, num_servers=1, capacity=2)
...
def test_simultaneous_arrivals() -> None:
    ...
    df['ArrivalTime'] = pd.to_datetime(df['ArrivalTime'], format='%H:%M')
    processed, _, _, metrics = priority(original_df=df, num_servers=2, capacity=3)
    assert not processed.empty
...
def test_output_types(test_data: pd.DataFrame) -> None:
    ...
    :return: None
    """
    processed, util_df, server_logs, metrics = priority(original_df=test_data, num_servers=2)
...
def test_priority() -> None:
    ...
    df = pd.DataFrame({
        ...
        'Age': [30, 40, 50],
        'TransactionTime': [10, 10, 10],
        'VulnerabilityScore': [0.1, 0.9, 0.5],
        'ArrivalTime': ['08:00', '08:00', '08:00']
    })
    df['ArrivalTime'] = pd.to_datetime(df['ArrivalTime'], format='%H:%M')

    processed, _, _, _ = priority(original_df=df, num_servers=1, capacity=3)
    assert processed.iloc[0]['ID'] == 2  # Highest vulnerability score (0.9)
...
def test_tie_breaker() -> None:
    ...
    df = pd.DataFrame({
        ...
        'Age': [25, 35],
        'TransactionTime': [10, 10],
        'VulnerabilityScore': [0.5, 0.5],
        'ArrivalTime': ['08:00', '08:05']
    })
    df['ArrivalTime'] = pd.to_datetime(df['ArrivalTime'], format='%H:%M')

    processed, _, _, _ = priority(original_df=df, num_servers=1, capacity=2)
    assert processed.iloc[0]['ID'] == 1  # Arrived earlier

def test_late_arrival() -> None:
    """
    Ensure that late-arrivals do not distrurb the queue
    :return: None
    """
    df = pd.DataFrame({
        'ID': [1, 2],
        'Gender': ['F', 'M'],
        'Age': [30, 50],
        'TransactionTime': [20, 10],
        'VulnerabilityScore': [0.3, 0.9],
        'ArrivalTime': ['08:00', '08:01']  # Person 2 arrives later but has higher score
    })
    df['ArrivalTime'] = pd.to_datetime(df['ArrivalTime'], format='%H:%M')

    processed, _, _, _ = priority(original_df=df, num_servers=1, capacity=2)
    assert processed.iloc[0]['ID'] == 1  # Person 1 starts service first

def test_priority_vs_fifo() -> None:
    """
    Compare with FIFO
    :return: None
    """
    df = pd.DataFrame({
        ...
        'Age': [25, 60],
        'TransactionTime': [10, 10],
        'VulnerabilityScore': [0.4, 0.9],
        'ArrivalTime': ['08:00', '08:00']
    })
    df['ArrivalTime'] = pd.to_datetime(df['ArrivalTime'], format='%H:%M')

    processed_p, _, _, _ = priority(original_df=df.copy(), num_servers=1, capacity=2)
    from src import fifo
    processed_f, _, _, _ = fifo(original_df=df.copy(), num_servers=1, capacity=2)

    assert processed_p.iloc[0]['ID'] != processed_f.iloc[0]['ID']

# TODO: Implement similar fifo checks for other models